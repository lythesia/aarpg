...
@onready var item_scroll: SnapScrollContainer = %ScrollContainer
...
func open_menu(shop_inventory: Array[SlotItemData]) -> void:
...
func populate_item_list(shop_inventory: Array[SlotItemData]) -> void:
    item_scroll.reset_scroll()
    ...
    for i in shop_inventory.size():
        var item: SlotItemData = shop_inventory[i]
...
func focused_item_changed(item: SlotItemData) -> void:
    if item:
        _update_item_details(item)

func purchase_item(item: SlotItemData) -> void:
...
func _update_item_details(item: SlotItemData) -> void:
...
func _get_item_hold_quantity(item: SlotItemData) -> int:
